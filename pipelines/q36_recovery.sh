...
say "A: student rollouts -> $S/rollouts.jsonl"
if [ ! -s "$S/rollouts.jsonl" ]; then gpu_free
  CUDA_VISIBLE_DEVICES=0,1,2,3 $VLLM src/distill/opd_gen.py --model "$STU" \
    --out "$S/rollouts.jsonl" --n 384 --k 2 --max-tokens 896 --tp 4 || { say "A FAILED"; exit 11; }
fi
say "A done: $(wc -l < "$S/rollouts.jsonl") rollouts"

say "B: teacher top-k logprobs -> $S/teacher_lp.npz"
if [ ! -s "$S/teacher_lp.npz" ]; then gpu_free
  CUDA_VISIBLE_DEVICES=0,1,2,3 $VLLM src/distill/opd_score.py --teacher "$TEA" \
    --rollouts "$S/rollouts.jsonl" --out "$S/teacher_lp.npz" --k 20 --max-len 1024 --tp 4 || { say "B FAILED"; exit 12; }
fi
say "B done"

say "C: FSDP2 train (replicated-LoRA, --checkpoint 0, dynamic) -> $S/adapter"
if [ ! -e "$S/adapter/adapter.pt" ]; then gpu_free
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,3 $TRUN --nproc_per_node=4 --master_port=29611 src/distill/opd_train_fsdp.py \
    ...
    --rank 16 --scale 2.0 --bits 3.0 --group 128 --epochs 2 --accum 8 --lr 5e-5 --max-len 1024 \
    --checkpoint 0 --ckpt-every 10 || { say "C FAILED"; exit 13; }
fi
