...
def main():
    ...
    # === DDP data parallelism (InstinctRazor patch): shard the calib set across ranks, keep the per-layer
    # sequential propagation, all-reduce(AVG) the rotation/scale/weight grads so ranks stay in lockstep,
    # replicate val so best-epoch selection is identical, save on rank 0. Launch with torchrun. ===
    import torch.distributed as dist
    _ddp = "RANK" in os.environ
    if _ddp:
        dist.init_process_group("nccl")
        _rank, _world = dist.get_rank(), dist.get_world_size()
        _local = int(os.environ.get("LOCAL_RANK", _rank))
        torch.cuda.set_device(_local)
        device = f"cuda:{_local}"
    else:
        _rank, _world, _local, device = 0, 1, 0, "cuda"
    def _is_main(): return _rank == 0

    wandb_run = setup_wandb(args) if _is_main() else None
    ...
    if _ddp:                       # each rank optimizes a disjoint slice of the calib set (data parallel)
        samples = samples[_rank::_world]
        print(f"[ddp] rank {_rank}/{_world} train shard: {samples.shape[0]} samples", flush=True)
    ...
    for layer_idx, layer in enumerate(tqdm(blocks)):
        ...
        if not all_files_exist:
            ...
            for step, step_params_dict in enumerate(params_to_optimize):
                ...
                if _ddp:   # all-reduce(AVG) grads of the just-enabled params each step -> ranks stay in sync
                    for _grp in optim_params:
                        for _p in _grp["params"]:
                            if _p.requires_grad and not getattr(_p, "_ddp_hooked", False):
                                _p.register_post_accumulate_grad_hook(
                                    lambda p: dist.all_reduce(p.grad, op=dist.ReduceOp.AVG))
                                _p._ddp_hooked = True
        ...
        # Save the optimized result (rank 0 only; params are identical across ranks)
        if _is_main():
